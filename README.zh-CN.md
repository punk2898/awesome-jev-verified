# Awesome Jev — 可验证版 [![Awesome](https://awesome.re/badge-flat.svg)](https://awesome.re)
...
**一、每条都有证据。** 收录与否不靠读 README 判断。下面每个仓库都在固定 commit 上被下载下来，在源码里搜索真正的 Jev 调用——API 端点、官方 SDK 引入、模型标识符，或 AI SDK 的 evaluate 接口。每条后面的 `kind` 徽标直接链到那个文件和行号。788 条里有 765 条过了这一关；另外 54 个搜到了但找不到代码证据的，放在 [`data/leads.json`](data/leads.json) 里存档，而不是混进列表。

**二、用实测替换宣传。** 大部分列表都在重复 TypeSafe 的官方说法——"0.1 秒以内""快 100 倍""彻底根除幻觉"。我们拿 2,390 道有标准答案的题，让 Jev 和四个对照模型一起跑了一遍。有些说法站得住，有几条站不住。

| 实测项 | Jev | GPT-4.1-mini | GPT-5.6 Sol |
| --- | --- | --- | --- |
| 是/否题准确率 | **92.5%** | 87.8% | 92.1% |
| 多选题准确率 | 77.3% | 74.3% | **82.5%** |
| 信心 ≥90% 却答错（800 题中） | **6** | 70 | 39 |
| 提示注入得手次数（80 次中） | **1** | 73 | 1 |
| 单题调用的中位延迟 | **456 ms** | 928 ms | 1661 ms |
| 跑完 2,390 题的花费 | **$0.049** | $0.246 | $4.27 |

逐条核对：

| 宣传说法 | 出处 | 实测结果 | 判定 |
| --- | --- | --- | --- |
| 概率经过校准 | docs | 是/否题 ECE 0.048，与 Sol（0.043）、Terra（0.051）同级，好于 Luna（0.105）。多选题 ECE 0.124，过度自信，与 Sol 相当。 | 部分属实 |
| 相似输入给出相近输出 | docs | 60 道换问法：0 次翻转（Sol 1、Terra 3、Luna 9）。120 组正反问法：仅 1 组自相矛盾（Sol 5、Terra 6、Luna 11）。五个模型中最稳。 | 属实 |
| 输出确定，可当强类型对象直接调用 | tweet | 同题重复 5 次：判断从未翻转，概率最大差 0.09。但仅 12/30 完全一致。Terra、Luna 会把同题答反（最大差 0.81、0.98）。 | 接近，但非逐位确定 |
| 延迟 0.1 秒以内，能放进 60 FPS 游戏循环 | tweet | 服务端中位 248ms，最快 189ms。40 次顺序调用无一低于 100ms。全程中位 456ms。 | 不属实 |
| 比语言模型快 100 倍 / 194 倍 | tweet / blog | 单题调用：比 GPT-5.6 快约 3–3.6 倍（456ms 对 1.3–1.7 秒），比 GPT-4.1-mini 快约 2 倍。一次打包 50 题时摊到每题约 5ms，数量级差距只在这里成立。 | 打包问才成立 |
| 输入每 10 亿 token 42 美元，输出免费 | docs | 1,154,813 输入 token 计费 $0.0485，费率一致。注意同样内容 Jev 计入的 token 约为 GPT 的 2 倍。 | 属实 |
| 比语言模型便宜 445 倍 | blog | 同样 2390 题：比 Sol 便宜 88 倍，比 Terra 45 倍，比 Luna 和 GPT-4.1-mini 约 5 倍。445 倍未复现。 | 看和谁比 |
| 彻底根除幻觉 | tweet | 1200 道标准数据集题中，36 次「信心 ≥90% 却答错」。是五个模型里最少的（Sol 84、Terra 88、Luna 140、GPT-4.1-mini 142），但不是零。 | 夸大 |
| 中文不如英文 | docs (TypeSafe's own caveat) | 同一批 150 道 XNLI：英文 79.3%，中文 68.7%。GPT-5.6 三个版本掉得更多（13–16 点），Jev 降幅（11 点）反而更小。 | 属实，但非 Jev 独有 |
...
- [官方仓库](#官方仓库) — **4**
- [SDK 与客户端](#sdk-与客户端) — **131**
- [Agent 工具](#agent-工具) — **154**
- [护栏与审查](#护栏与审查) — **47**
- [上下文工程](#上下文工程) — **25**
- [浏览器与电脑操作](#浏览器与电脑操作) — **42**
- [基础设施](#基础设施) — **31**
- [搜索与检索](#搜索与检索) — **25**
- [应用产品](#应用产品) — **106**
- [游戏、机器人与仿真](#游戏机器人与仿真) — **49**
- [开源复现](#开源复现) — **35**
- [评测与研究](#评测与研究) — **83**
- [榜单与资源](#榜单与资源) — **56**

每个分类在这里只展示 star 最高的 10 个，完整名单点进去一页就是。star 数读取于 2026-09-20，它衡量的是关注度，不是质量。
...
被收录只说明一件事：在 2026-09-20 这一天，这个仓库里有调用 Jev 的代码。它**不是**对质量、安全性、许可证或"这玩意儿能不能跑"的背书。这里不少仓库只有几天大，是在发布热潮里推上来的。在依赖任何一个之前，请自己读代码、看许可证、跑一遍。

## 列表

### 官方仓库

TypeSafe AI 官方发布的仓库。 · **4** 个
...
→ 完整列表: [categories/official.md](categories/official.md)

### SDK 与客户端

各语言的客户端、绑定和命令行工具，用来调用 Jev。 · **131** 个
...
→ 其余 121 个见 [categories/sdk.md](categories/sdk.md)

### Agent 工具

把 Jev 接进编码 agent 的插件、MCP server 和 skill。 · **154** 个
...
→ 其余 144 个见 [categories/agent.md](categories/agent.md)

### 护栏与审查

放行、拦截、标记或升级：安全闸门、内容审核、代码审查。 · **47** 个
...
→ 其余 37 个见 [categories/guardrail.md](categories/guardrail.md)

### 上下文工程

压缩、裁剪、记忆，决定 agent 该读什么、留什么。 · **25** 个
...
→ 其余 15 个见 [categories/context.md](categories/context.md)

### 浏览器与电脑操作

由类型化决策驱动的浏览器自动化、GUI 与设备控制。 · **42** 个
...
→ 其余 32 个见 [categories/browser.md](categories/browser.md)

### 基础设施

网关、路由、缓存、数据库扩展和自托管端点。 · **31** 个
...
→ 其余 21 个见 [categories/infra.md](categories/infra.md)

### 搜索与检索

语义 grep、排序、过滤，以及对文档和数据的分类。 · **25** 个
...
→ 其余 15 个见 [categories/retrieval.md](categories/retrieval.md)

### 应用产品

面向最终用户的产品：交易、社交、招聘、智能家居、效率工具。 · **106** 个
...
→ 其余 96 个见 [categories/app.md](categories/app.md)

### 游戏、机器人与仿真

游戏、仿真、无人机等具身或好玩的演示。 · **49** 个
...
→ 其余 39 个见 [categories/game.md](categories/game.md)

### 开源复现

开源复刻、替代的 System One 模型和本地运行时。 · **35** 个
...
→ 其余 25 个见 [categories/repro.md](categories/repro.md)

### 评测与研究

评估、校准研究和测试框架，用来量化模型表现。 · **83** 个
...
→ 其余 73 个见 [categories/research.md](categories/research.md)

### 榜单与资源

其他精选列表、目录、指南和 cookbook。 · **56** 个
...
→ 其余 46 个见 [categories/list.md](categories/list.md)
